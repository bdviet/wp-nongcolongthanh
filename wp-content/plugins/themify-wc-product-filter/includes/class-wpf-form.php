...
class WPF_Form {
    ...
    public function save_themplate(array $post) {
        ...
        if (!empty($post['layout'])) {
            ...
            $_keys = array('name', 'empty', 'group','reset_button', 'type', 'page', 'sort', 'pagination', 'posts_per_page', 'result', 'out_of_stock', 'scroll', 'result_type', 'tax_relation', 'pagination_type');
            foreach ($_keys as $k) {
                if (!empty($post[$k])) {
                            ...
                            <div class="wpf_item wpf_item_<?php echo $type ?>">
                                ...
                                <?php echo $view ?>
									<?php if('group' === $reset_btn && !in_array($type,$non_groups) ): ?>
                                        <div class="wpf_reset_btn"><input type="reset" value="<?php _e('Clear','wpf'); ?>"/></div>
									<?php endif; ?>
