...
class Kv_Products_List extends WP_List_Table
{
    ...
    public static function get_total()
    {
        ...
        $query = "SELECT count(*) as total FROM `{$wpdb->prefix}posts` AS a INNER JOIN `{$wpdb->prefix}kiotviet_sync_products` AS b ON a.ID = b.product_id WHERE a.post_status = 'publish' AND a.post_type = 'product' AND b.retailer = '" . kiotviet_sync_get_data('retailer', "") . "'";
    ...
    public static function get_products($per_page, $current_page)
    {
        ...
        $query = "SELECT * FROM `{$wpdb->prefix}posts` AS a INNER JOIN `{$wpdb->prefix}kiotviet_sync_products` AS b ON a.ID = b.product_id WHERE a.post_status = 'publish' AND a.post_type = 'product' AND b.retailer = '" . kiotviet_sync_get_data('retailer', "") . "'";
        if (array_key_exists('s', $_REQUEST)) {
            $query .= " AND a.post_title LIKE '%" . sanitize_text_field($_REQUEST['s']) . "%'";
        }
