<?php
/**
 * Plugin Name: Service Giaohangtietkiem
 * Description: Plugin hỗ trợ đơn vị giao vận Giaohangtietkiem cho WooCommerce. Khách hàng chủ động chọn đơn vị giao vận và các gói giao vận ( Nhanh, Chuẩn, Tiết Kiệm ) tuỳ theo hầu bao của mình, việc này tạo sự tin tưởng cho người mua vì công khai chi phí ship giúp tăng tỉ lệ đặt hàng cho quản trị shop. Quản trị shop dễ dàng đăng vận đơn lên các đơn vị giao vận tuỳ theo lựa chọn của khách hàng khi đặt hàng chỉ với 1 Click, cùng với đó là tra cứu trạng thái vận đơn ngay từ trang quản trị.
 * Version: 1.0.0
 * Author: Giaohangtietkiem - vinhtq@ghtk.vn
 * Author URI: https://ghtk.vn
 * License: GNU General Public License v3 or later
 * License URI: https://www.gnu.org/licenses/gpl-3.0.html
 *
 * Shipping Vietnam Woocommerce plugin, Copyright 2020 Hoang Quoc Long
...
define( 'SVW_CITIES', file_get_contents( SVW_DIR_PATH . 'assets/json/cities.json') );
define( 'SVW_DISTRICTS', file_get_contents( SVW_DIR_PATH . 'assets/json/districts.json') );
define( 'SVW_WARDS', file_get_contents( SVW_DIR_PATH . 'assets/json/wards.json') );
...
class Shipping_Vietnam_Woocommerce {
    ...
    function admin_enqueue_script() {
        ...
        wp_enqueue_style( 'svw-admin', SVW_DIR.'assets/css/ghtk.css', false, '1.0.0' );

        if ( $screen_id == 'woocommerce_page_wc-settings' || $screen_id == 'shop_order' ) {
            wp_enqueue_script( 'svw-admin', SVW_DIR.'assets/js/ghtk.js', array('jquery'), false, true );
