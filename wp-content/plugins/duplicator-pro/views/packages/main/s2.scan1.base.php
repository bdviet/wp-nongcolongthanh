...
<form id="form-duplicator" method="post" action="<?php echo $package_list_url ?>">
	...
	<div id="dpro-confirm-area">
		...
		echo '<br/> ';
		DUP_PRO_U::esc_html_e('At least one or more checkboxes was checked in "Quick Filters".') ?><br/>
			...
			success: function (respData, textStatus, xHr) {
				...
				if(data.ScanStatus !== undefined && data.ScanStatus == 'running'){
                    ...
                    if (status == DupPro.Pack.WebServiceStatus.Pass) {
                        ...
                        message  = '<b><?php DUP_PRO_U::esc_html_e("Please Retry:") ?></b><br/>';
                        message += '<?php DUP_PRO_U::esc_html_e("Unable to perform a full scan and read JSON file, please try the following actions.") ?><br/>';
                        message += '<?php DUP_PRO_U::esc_html_e("1. Go back and create a root path directory filter to validate the site is scan-able.") ?><br/>';
                        message += '<?php DUP_PRO_U::esc_html_e("2. Continue to add/remove filters to isolate which path is causing issues.") ?><br/>';
                        message += '<?php DUP_PRO_U::esc_html_e("3. This message will go away once the correct filters are applied.") ?><br/><br/>';

                        message += '<b><?php DUP_PRO_U::esc_html_e("Common Issues:") ?></b><br/>';
                        message += '<?php DUP_PRO_U::esc_html_e("- On some budget hosts scanning over 30k files can lead to timeout/gateway issues. Consider scanning only your main WordPress site and avoid trying to backup other external directories.") ?><br/>';
                        message += '<?php DUP_PRO_U::esc_html_e("- Symbolic link recursion can cause timeouts.  Ask your server admin if any are present in the scan path.  If they are add the full path as a filter and try running the scan again.") ?><br/><br/>';

                        message += '<b><?php DUP_PRO_U::esc_html_e("Details:") ?></b><br/>';
                        message += '<?php DUP_PRO_U::esc_html_e("JSON Service:") ?> /wp-admin/admin-ajax.php?action=duplicator_pro_package_scan<br/>';
                        message += '<?php DUP_PRO_U::esc_html_e("Scan Path:") ?> [<?php echo esc_html(rtrim(DUPLICATOR_PRO_WPROOTPATH, '/')); ?>]<br/><br/>';
	...
	DupPro.Pack.loadScanData = function (data)
	{
		try {
            ...
            if(data.SRV.Brand.LogoImageExists)
                ...
                $("#data-srv-brand-note").html("<?php DUP_PRO_U::esc_html_e("WARNING! Logo images no longer can be found inside brand. Please edit this brand and place new images. After that you can build yur package with this brand.") ?>");
